...
from .update_tle import TLE_GROUPS, client, do_retry, headers
            ...
            while True:
                ...
                if self._satrec_array:
                    try:
                        ...
                        fr_arr = np.array([0.0])
                        _, r, v = self._satrec_array.sgp4(jd_arr, fr_arr)
                        ...
                        lat_rad = np.arctan2(
                            z + EP2 * WGS84_B * (sin_tb**3),
                            p - E2 * WGS84_A * (cos_tb**3),
                        )
...
async def update_tle(manager: SatelliteManager) -> None:
    for group in TLE_GROUPS:
        ...
        try:
            ...
            print(f"[TLE] Error updating group {group}: {e}, {e!r}", flush=True)
